// Test through the actual renderFragment function

import { renderFragment, extractUrls, validateUrls } from "../../src/render"
...
console.log("=== Calling renderFragment... ===")
...
if (missing.length > 0) {
  ...
  // Check if URLs are partially present
  console.log("\n=== Partial match check ===")
  for (const url of missing) {
    const domain = url.split("/")[2]
    if (result.includes(domain)) {
      console.log(`Domain '${domain}' found in output`)
      // Find what URL is there for this domain
      const regex = new RegExp(`href="[^"]*${domain}[^"]*"`, "g")
      const matches = result.match(regex)
      if (matches) {
        console.log(`  Actual URLs: ${matches.join(", ")}`)
      }
    } else {
      console.log(`Domain '${domain}' NOT found in output`)
    }
  }
} else {
  console.log("\n✓ All URLs preserved!")
